...
- [Introduction](#introduction)
  ...
  - [Using ROCCC for Data
    Creditbility](#using-roccc-for-data-creditbility)
  - [Brief Explanation of Sections](#brief-explanation-of-sections)
...
- [Part 1: Daily Level Analysis](#part-1-daily-level-analysis)
  ...
  - [Data Visualizations and Plots](#data-visualizations-and-plots)
    ...
    - [Identifying Relationships](#identifying-relationships)
      ...
      - [Impact of Sedentary Behavior on Sleep Quality and
        Activity](#impact-of-sedentary-behavior-on-sleep-quality-and-activity)
      - [Does Increased Activity Mean a Compromise on
        Sleep?](#does-increased-activity-mean-a-compromise-on-sleep)
      - [What Influences Calorie
        Expenditure](#what-influences-calorie-expenditure)
...
- [Recommendations and Conculsion](#recommendations-and-conculsion)
  - [Recommendations based on Daily
    Trends](#recommendations-based-on-daily-trends)
  - [Recommendations based on
    Interrelationships](#recommendations-based-on-interrelationships)
  - [Recommendations based on Hourly
    Trends](#recommendations-based-on-hourly-trends)
...
health-focused smart products for women and was co-founded by Urška
Sršen and Sando Mur.
...
The data set, hosted on Kaggle, contains personal tracker data from 30
...
The data set has a limited sample size (**33** respondents) and lacks
critical demographic information such as age, gender, and occupation.
Additionally, the weight data set is too small (8 respondents) to
provide statistically significant insights and will be excluded.
...
## Using ROCCC for Data Creditbility

The ROCCC (Reliable, Original, Comprehensive, Current and Cited) method
can be applied to evaluate the data’s credibility. The results are as
following:

- **Reliability**: **Low/Limited in scope** due to the very small sample
  size (33) and the absence of demographic data or any indicators
  related to occupation, health or disabilities. The timespan covered is
  also not enough to capture any seasonal trends or differences.
- **Original**: **Low/Limited in scope** as the data is Third-party data
  collected by Amazon Mechanical Turk
- **Comprehensive**: **Medium-Low/Somewhat Limited in scope**. The
  dataset contains useful metrics, including calorie count, steps,
  sleep, and intensity levels. However, it is limited by a lack of
  demographic data, a small number of respondents, and a restricted
  tracking period.
- **Current**: **Low/Limited in scope**. This data was collected 10
  years ago and doesn’t capture current trends and lifestyles
- **Cited**: **High**. The data set is well cited on Kaggle
...
- The Data Processing and Cleaning Section will shortlist, clean, and
  join daily level data for analysis
- The Data Visualization and Plots Section will first focus on
  understanding the individual distributions and composition of
  variables and then proceed to identifying any key relationships
  between them
...
- The Data Processing and Cleaning Section will shortlist, clean, and
  join hourly level data for analysis
- The Data Visualization and Plots Section will review key summary
  statistics and examine how the distributions of the three indicators
  fluctuate throughout the hours of the day.

At the end, this case study will conclude by summarizing the key
findings, insights, and recommendations aimed at helping the Bellabeat
marketing team expand its market share and more effectively address
customer needs.
...
SQL will be used for all Data Processing and Cleaning up to and
including the Join <br>
...
- The row corresponding to the ‘ID’ field is highlighted in **green**,
  as this column will serve as one of the keys for joining tables.
...
- 0 records are returned when ActivityDate in the Activity table is
  *not* the same as ActivityDay in the Steps table.
- 0 records are returned when ActivityDate in the Activity table is
...
The code below checks that RecordedDate stores the correct values from
...
-- When Day is in Single Digits:
-- Digits 5-8 as YYYY
-- 0 and First digit for MM 
-- 0 and Third Digit  as DD 

-- When Day is in Double Digits:
-- Digits 6-9 as YYYY
-- 0 and First digit for MM 
-- Third and Fourth Digit as DD 
...
The code brow checks that RecordedDate stores the correct values from
...
for the relevant columns from the Fitabase data dictionary available at
this link:
...
- We will exclude rows when Steps are less than 1 (equal to 0) as these
  are likely to be days when the watch wasn’t worn or it didn’t record
  anything
...
WHERE 
  A.TotalSteps > 0 -- drop rows where Total Steps = 0
```

### Review Joined Data

Lets now the first few rows of the joined data frame for some quick
...
more comprehensive understanding of the data set. We will split the view
...
- Daily average for calories burned, 2,361, aligns with standard dietary
  guidelines.
- Interestingly, the average step count, 8319 is well within the typical
  5,000–7,000 step range, suggesting an active user base. Source:
...
- Lightly Active Minutes are significantly higher than both Fairly
  ...
  at approximately 210, is about 14 times greater than Fairly Active
...
- The mean for Minutes Asleep is 419 minutes, which is approximately 7
  hours and aligns with the standard recommended value.
- Missing values are only observed for columns from the Sleep table
- The data set sourced from the Sleep table exhibits 453 missing values.
...
First lets analyze calories with a histogram using a custom function to
visualize the distribution.

**Distribution of Total Calories**

``` r
hist_func <- function(column,indicator,unitdiv,fillcol,bw) {
# column for relevant column name
# indicator for descriptive name for titles and labels
# unitdiv to convert any values eg minutes to hours. If nothing needs to be 
  #converted then this will be set as 1
# fillcol for color of histogram
# bw for binwidth
  
# This function:
# Plots a histogram for the specified column after dividing it by a divisor
ggplot(daily_activity_sleep, aes(.data[[column]]/unitdiv)) +
 geom_histogram(fill = fillcol, color = 'white', binwidth=bw) +
  labs(title = paste0("Histogram of " ,indicator), 
  x = indicator, 
  y = "Frequency") 
}
hist_func("Calories","Calories (kcal)",1,'salmon',200)
```
...
burned using a custom function.

``` r
daily_trend_func <- function(column,indicator) {
  
  # column for relevant column name
  # indicator for descriptive name for titles and labels
  
  # The function:
  # Groups the daily_activity_sleep dataframe by Date and Id. 
  # Calculates the daily mean of the specified column and divides it by the divisor 
  # Plots the daily means against dates 

daily_activity_sleep %>% group_by(RecordedDate,DayType) %>% 
  summarise(daily_avg= mean(.data[[column]]), .groups = 'drop') %>% # Calculate mean per day
  ...
  labs(
    ...
    y = paste0("Average ",indicator), # New y-axis title
    title = paste0("Average ",indicator," Per Day"), # New title
    color="Day Type", # New legend title
    caption = "Total Respondents: 33, Year: 2016"
  )
}

daily_trend_func("Calories","Calories (kcal)")
```
...
``` r
hist_func("Steps","Total Steps",1,'salmon',1250)
```
...
Additionally the distribution for Total Steps suggests that the
participant group includes individuals who are both significantly high
and significantly low performers as:

1.  High Performers (Enthusiasts): The peak of the distribution is
    observed at 10,000 steps. There is also notable number of
    observations recording well over its. These high-value observations
    raise the average step count to approximately 8319 (refer to summary
    statistics), placing it within the 5,000–7,000 range. Bellabeat can
    target these enthusiasts by recommending high intensity workout
    plans and fitness goals

2.  Low Performers : Conversely, good portion of the distribution is
    also concentrated on the left of 10,000. This suggests a substantial
    number of users are not meeting the recommended daily goal of
    5,000–7,000 steps. Bellabeat could consider targeting these users
    with daily reminders amd low intense, easy workout plans.

Let’s explore this further by segmenting users based on Average Daily
Step Count
...
``` r
daily_activity_sleep %>%
  group_by(Id) %>% ## Group rows for each individual
  summarize(mean_daily_steps = mean(Steps)) %>% ##calculate mean of daily steps
  mutate(user_type = case_when(       ## categorize individuals 
    mean_daily_steps < 5000 ~ "Sedentary",
    mean_daily_steps >= 5000 & mean_daily_steps < 7499 ~ "Lightly Active",
    mean_daily_steps >= 7500 & mean_daily_steps < 9999 ~ "Fairly Active",
    mean_daily_steps >= 10000 ~ "Very Active"
  )) %>%
  count(user_type) %>%  ## count occurrences of each user type
  mutate(percentage = n*100 / sum(n)) %>% ## calculate percentage of each occurrence
ggplot(aes(x = "", y = percentage, fill = user_type)) +
  geom_bar(stat = "identity", width = 1) +
  coord_polar("y", start = 0) + 
  theme_void() + # Removes background and gridlines for a clean look
  labs(
    title = "Composition of User Activity for Average Daily Steps",
    subtitle = "Classification based on Preliminary Pedometer Indices 
    by Catrine Tudor-Locke and David R Bassett Jr*",
       caption= "*https://pubmed.ncbi.nlm.nih.gov/14715035/",
       fill='User Type') +
...
<img src="Fitbit_DetailedAnalysis_For_Bellabeat_files/figure-gfm/seg_users-1.png" alt="" style="display: block; margin: auto;" />
The user segmentation data reveals that a significant portion of the
participants are categorized as Fairly or Lightly Active. However, a
considerable segment—**21.2% of the population**—is Sedentary, averaging
less than 5,000 steps per day. This Sedentary group represents a
critical target audience for fitness companies like Bellabeat.

The varied nature of the population highlighted in the User Segmentation
chart emphasizes that a one-size-fits-all strategy will be ineffective
for Bellabeat. To successfully meet the diverse needs of the users, a
varied and targeted approach is necessary.
...
Similar to Calories, the average of daily steps also remained relatively
stable throughout April but the variation in the points is a lot more
than it was for Calories.

#### Analyzing Total Active Minutes and Sedantary Minutes

Lets next analyze the Minutes variables. Recall that in the Join
statement Total Active Minutes was created as the sum of Fairly Active
Minutes, Lightly Active Minutes and Very Active Minutes

**Composition of Total Active Min**

Lets first review the composition of Total Active Minutes using a custom
function.

``` r
 pie_plot_func <- function(cols_plot,indicator) {
  
  # cols plot to identify list of columns
  # indicator for descriptive name for titles and labels
  
  # The function:
  # Calculates the sum of the specified columns and their percentages as a portion of the whole. 
  # Creates A pie chart is to visualize the proportions. 
  # Means for respective categories are also calculated and displayed shown.

  
  # Use the apply function to sum all rows in each of the columns
  # Calculate percentage by dividing each sum value with the sum 
  # Update Category to include means by dividing each sum values with total rows
  # in the joined data set
  
  pie <- daily_activity_sleep[,cols_plot] %>% 
  apply(2,sum) %>% as.data.frame() %>% setNames("value") %>%  
  rownames_to_column("category") %>% 
  mutate(percentage = value / sum(value) * 100, 
         means=value/nrow(daily_activity_sleep), 
         category = paste0(category, " (", round(means, 1),")")) %>% 
  arrange(value) %>% 
  ggplot(aes(x = "", y = value, fill = category)) + #Plot the data using geom_bar
  ...
  labs(title = paste0("Composition of ",indicator), 
       fill='Category (Mean)') + #Add title and legend
  geom_text(aes(label = paste0(round(percentage, 1), "%")), 
...
pie_plot_func(c("FairlyActMin","LightlyActMin","VeryActMin"),"Total Active Minutes")
```
...
Lightly Active Minutes (210 minutes or 3.5 hours) is notably higher than
the averages for Fairly Active Minutes (~ 15 minutes) and Very Active
Minutes (23 minutes). Overall, the data again indicates that most
respondents are primarily lightly active
...
``` r
hist_func("TotalActMin","Total Activity (Hours)",60,'salmon',0.5)
```

<img src="Fitbit_DetailedAnalysis_For_Bellabeat_files/figure-gfm/hist_totactmin-1.png" alt="" style="display: block; margin: auto;" />
The distribution of Total Activity exhibits a peak around 3.75, which
suggests the presence of a generally active user base. This group
represents a key target audience for Bellabeat, offering an opportunity
to increase engagement by recommending tailored workout plans and
specific fitness goals.

As shown in the pie-chart earlier, Total Active hours compose mainly of
Lightly Active Hours and Bellabeat can encourage

**Daily High Activity Trend**

Let’s now examine if there are any time-based differences on High
Activity or Very Active Minutes.

``` r
daily_trend_func("VeryActMin","Very Active Minutes")
```
...
Unlike the other indicators, the Average of Very Active Minutes rises
and then falls over time. This fall may be attributed to **Rising Summer
Temperatures** i.e. As summer progresses, people may be less inclined to
participate in intensive activities especially outdoor ones due to
increased heat. Bellabeat could address this by promoting and
recommending indoor workouts and activity options during the summer
months.

**Composition of Active + Sedentary Minutes**

Lets next review the composition of Active + Sedentary Minutes as a sum
of Total Active Minutes and Sedentary Minutes.

``` r
# Select TotalActiveMinutes and SedentaryMinutes
pie_plot_func(c("TotalActMin","SedMin"),"Active + Sedentary Minutes")
```
...
Sedentary Minutes constitute a striking 79.4% of the total time. This
high proportion directly leads to a very high average of around 956
minutes (or 16 hours) for Sedentary Minutes, significantly overshadowing
the average of around 248 minutes (or 3.8 hours) for Total Active
Minutes. The data strongly suggests a considerably sedentary lifestyle
among the users, which is a key area Bellabeat should target for
intervention to promote lower sedentary behavior.

**Distribution of Sedentary Hours**

Next, lets use a histogram to visualize the distribution of Sedentary
hours. Minutes are converted to hours for easier readability

``` r
hist_func("SedMin","Sedentary Hours",60,'midnightblue',1)
```

<img src="Fitbit_DetailedAnalysis_For_Bellabeat_files/figure-gfm/hist_sedentary-1.png" alt="" style="display: block; margin: auto;" />
The data shows a Peak for Sedentary Hours at 12 hours which represents a
very high level as compared to the recommended time limit of 300 minutes
or 6 hours\*. This highlights that a significant portion of users spend
more than half of their day in a sedentary lifestyle.Bellabeat should
target this group with reminders to engage in activity when sedentary
hours surpass a specific threshold. Suggestions such as “Time to take a
walk” or “Time for some stretches”,“Time to Stand” could be beneficial
to convert sedentary users into active ones.
...
pie_plot_func(c("LightActDist","ModActDist","VeryActDist"),"Total Distance (km)")
```
...
with an average of 3.6 km. This is followed by VeryActiveDistance at
approximately 28% (mean of 1.6 km) and ModeratelyActiveDistance at 10%
...
``` r
hist_func("TotalDist","Total Active Distance",1,'salmon',1)
```
...
Furthermore, the distribution for Total Steps suggests that the
participant group includes individuals who are both significantly high
and significantly low performers as:

1.  High Performers (Enthusiasts): The data exhibits a second peak
    around 7 km (4 miles). These relatively high observations elevate
    the average daily distance to approximately 6 km (as indicated in
    the summary statistics). Bellabeat has the opportunity to target
    these enthusiasts by recommending personalized workout plans and
    fitness goals to facilitate further improvemen

2.  Low Performers (Low Outliers): A substantial portion of the
    distribution is concentrated below 4 km (2.5 miles), which is the
    recommended threshold for classifying an individual as “active”\*.
    Bellabeat may consider engaging these users with daily reminders.
...
``` r
daily_trend_func("VeryActDist","Active Distance (km)")
```
...
Similar to Intensive Activity, the average of Total Active Distance
rises and then falls over time. This fall fall may be attributed to
**Rising Summer Temperatures** as discussed earlier.
...
``` r
hist_func("MinAsleep","Total Hours Alseep",60,'midnightblue',1)
```

<img src="Fitbit_DetailedAnalysis_For_Bellabeat_files/figure-gfm/hist_asleep-1.png" alt="" style="display: block; margin: auto;" />
The majority of users report approximately 7 of sleep per day, which
aligns with general recommendations for adults\*. As noted in the Data
Processing and Cleaning section, most individuals sleep only once per
day, with the TotalSleepRecords variable indicating a value of 1 for 88%
of observations.

\*<https://www.sleepfoundation.org/how-sleep-works/how-much-sleep-do-we-really-need>

For users at the extremes of the distribution (those sleeping less than
5 hours or more than 9 hours), Bellabeat may implement reminders or
alarms to promote adherence to the recommended guidelines. Bellabeat can
also help users set up sleep schedules.

### Identifying Relationships

Having established a clear understanding of the individual distributions
and composition of the variables, we shall now proceed to identify their
interrelationships. This section will focus on generating a correlation
matrix heat map and scatter plots to discern key relationships. We will
also evaluate any impact of the type of the day (Weekend vs Weekday) on
fitness trends.

**Key Data Handling Decisions:**

- Focus on Totals: The analysis will prioritize overall trends in
  activity and fitness. Therefore, individual component variables such
  as Lightly/Fairly/Very Active Minutes and Light/Moderately/Very Active
  Distance will be excluded in favor of their respective totals.

- Redundant Variables: To streamline the analysis, variables that are
  expected to be highly correlated or equivalent will be reduced:

  - Total Distance vs. Total Steps: We anticipate Total Distance to be
    equivalent to Total Steps, so only Total Steps will be retained.

  - Time In Bed vs. Minutes Asleep: We anticipate Total Time in Bed to
    be equivalent to Total Minutes Asleep, so only Minutes Asleep will
    be retained

To confirm the equivalence of the last two points (Distance/Steps and
Time In Bed/Minutes Asleep), correlation coefficients will be
calculated. The code following this section will display these
coefficients.

``` r
# For each pair:
# Calculate the 2X2 correlation matrix and obtain the second element in the first row 
...
```
    ...
    ## Correlation between Total Distance and Total Steps: 0.97
    ## Correlation between Time In Bed and Minutes Asleep: 0.93

The results derived from the preceding code confirm the redundancy of
the variables “Total Distance” and “Time in Bed” for subsequent
analysis.
...
We will use DayType to evaluate any differences in their relationships
due to the type of the day.

#### Correlation Matrix Heat Map

A heat map serves as an effective method for identifying strong linear
relationships. While a low correlation coefficient does not preclude the
existence of a relationship (as a non-linear one may be present), a high
correlation coefficient unequivocally provides evidence for the presence
of one.

Therefore, the initial step will be to generate a heat map to quickly
visualize the correlations. The code below creates a correlation matrix
and visualizes that using a heat map
...
The analysis reveals several important relationships within the fitness
data:

- Negative Correlations: The variables “Sedentary Minutes” and “Minutes
  Asleep” demonstrate negative correlations with all other variables
  (indicated by purple values). However, it is worth noting that the
  correlations for “Minutes Asleep,” as well as the correlation between
  “Sedentary Minutes” and “Calories,” are all very close to zero.
  - Sedentary Behavior Impact: Sedentary Minutes are strongly linked to
    Minutes Asleep and, moderately, to Total Active Minutes. This
    suggests that a **higher amount of sedentary behavior is associated
    with lower sleep quality and decreased activity levels**. This
    represents a potential area for Bellabeat to focus its product
    features and messaging. Our upcoming section will delve deeper into
    the connection between Sedentary Behavior and its effects on both
    Sleep Quality and Activity.
  - Near Zero Correlations: **Minutes Asleep shows surprisingly low, or
    near-zero, correlations with both Calories and Activity**. Given
    that correlation only measures linear relationships, further
    investigation is necessary to determine if any other type of
    relationship exists between these variables.
- Positive Correlations (Activity and Calorie Burning): Total Active
  Minutes, Steps, and Calories have moderate or strong positive
  relationships with each other (indicated by orange values). While it
  is expected that more steps lead to higher activity, the data also
  demonstrates that **higher activity levels and step counts have a
  moderately strong effect on calorie burning**. This connection is
  another opportunity for Bellabeat to leverage in its advertising
  campaigns, as it provides a strong incentive for users to increase
  their physical activity. The next section will explore these key
...
corrplot_funtion <- function(daytype) {
  ...
  # Visualizes the matrix using a heat map
...
**Key Observations from Correlation Heat maps:**

- Sedentary Behavior and Sleep: All correlations related to sedentary
  minutes are stronger on weekends than weekdays. Notably, the
  **negative correlation between Minutes Asleep and sedentary minutes is
  significantly stronger on weekends**. This suggests that Bellabeat
  could benefit from promoting increased activity on weekends to help
  reduce sedentary behavior.
- Calories and Activity: All correlations related to calories are also
  stronger on weekends than weekdays. The **correlation between Calories
  and Total Active Minutes is much stronger on Weekends**. This
  observation prompts the suggestion that Bellabeat should follow a more
  active policy on weekends to encourage users to boost calorie burn via
  higher activity.

**Research Questions for Further Exploration:**

The analysis of all three heat maps leads to several important research
questions, which are summarized below:

1.  How does sedentary behavior impact sleep quality and overall
    activity? Is there a difference in the impact on sleep quality
    between weekdays and weekends?
2.  Does increased activity result in a compromise on sleep quality or
    duration?
3.  What are the primary factors that influence calorie expenditure? Is
    there a difference in the impact of Activity on Calories between
...
We will first try to answer, how does sedentary behavior impact sleep
quality and overall activity? We will also see if there’s a difference
in the impact on sleep quality between weekdays and weekends.

**Total Minutes Asleep and Sedentary Minutes**

Sedentary Minutes and Minutes Asleep had an overall correlation of
co-efficient of -0.6. When disintegrated into weekends and weekdays, it
was -0.51 for weekdays and -0.75 for weekends. This means that on
weekends 0.75^2 or 56% of the variation in Minutes Asleep is explained
by Sedentary Minutes. On weekdays it is 0.51^2 or 26%.

Let’s plot the two variables together and see this further

``` r
daytype_plot <- function(x_col,y_col,x_indicator,y_indicator) {
  
  # x_col as the x variable
  # y_col as the y variable
  # x_indicator as the descriptive name for the x variable
  # y_indicator as the descriptive name for the y variable
  
  # This function plots the x variable and the y variable using a scatter plot
  # and two smooth lines (one for Weekend and one for Weekdays)
  
  ggplot(daily_activity_sleep) + 
  geom_jitter(aes(x=.data[[x_col]],y=.data[[y_col]]),color='slategray') + 
  geom_smooth(aes(x=.data[[x_col]],.data[[y_col]],color=DayType),method='gam') +
  labs(
    x = x_indicator, # New x-axis title
    y = y_indicator, # New y-axis title
    color = "Day Type", # New legend title 
    title = paste0(y_indicator," vs. ",x_indicator) # New title
  )
}

daytype_plot("SedMin","MinAsleep","Sedentary Minutes","Total Minutes Asleep")
```
...
**The Relationship Between Sedentary Behavior and Poor Sleep Quality**

The plots confirms the results of the correlation coefficient. There is
definitely a strong inverse relationship between the two variables; as
sedentary minutes rise, the time spent sleeping decreases. Prolonged
sedentary behavior can significantly contribute to poor sleep quality
through several mechanisms:

- Weakened Sleep Drive: Insufficient physical activity reduces the
  body’s natural need for sleep, leading to difficulty initiating and
  maintaining sleep.
- Circadian Rhythm Disruption: Regular movement is essential for
  regulating the body’s sleep-wake cycle. A lack of activity can weaken
  this cycle, resulting in fragmented sleep patterns.
- Melatonin Suppression from Screen Time: Many sedentary activities,
  such as watching TV or using computers, involve exposure to bright,
  blue light. This light suppresses the production of melatonin, the
  hormone necessary for falling asleep.
- Mental and Physical Health Factors: Inactivity is linked to higher
  levels of stress, anxiety, and depression, which are common causes of
  sleep problems. Furthermore, physical discomfort from extended sitting
  can disrupt sleep.
...
**Potential Impact on Weekends and Suggested Interventions:**

We also observe a much steeper relationship on weekends vs weekdays. The
negative effects are more pronounced on weekends, possibly due to an
increase in late-night sedentary activities like reading, social media
use, and watching movies.

Given this pattern, Bellabeat could implement the following:

- Sleep Reminders: Send notifications to users to encourage sleep when
  late-night sedentary behavior is detected.
- Screen Time Management: Track screen usage and send reminders to take
  breaks when use is prolonged.

**Total Active Minutes and Sedentary Minutes**

The heat map also suggested a moderately negative relationship between
Sedentary Minutes and Activity. The scatter plot below demonstrates this
relationship

``` r
combined_plot <- function(x_col,y_col,x_indicator,y_indicator) {
  ...
  ggplot(daily_activity_sleep) + 
  geom_jitter(aes(x=.data[[x_col]],y=.data[[y_col]]),color='slategray') + 
  geom_smooth(aes(x=.data[[x_col]],.data[[y_col]]),color='salmon',method='gam') +
  labs(
    x = x_indicator, # New x-axis title
    y = y_indicator, # New y-axis title
    title = paste0(y_indicator," vs. ",x_indicator) # New title
...
combined_plot("SedMin","TotalActMin","Sedentary Minutes","Total Active Minutes")
```

<img src="Fitbit_DetailedAnalysis_For_Bellabeat_files/figure-gfm/totalactmin_vs_sedmin-1.png" alt="" style="display: block; margin: auto;" />

The scatter plot data, though split, consistently reveals a negative
correlation between total active minutes and sedentary minutes. This
logical relationship shows that as sedentary behavior (sitting or
inactivity) increases, physical activity decreases.

This negative trend is particularly evident when sedentary time exceeds
1000 minutes (approximately 16 hours), which aligns with the calculated
mean average.

Based on this strong inverse relationship, it is vital for Bellabeat to
integrate actionable activity suggestions into reminders aimed at
reducing sedentary time. For instance, messages could include: “You have
been sitting for too long, time to take a walk,” or “Trouble having
sleep? Time to put the phone down and do some stretches.”

#### Does Increased Activity Mean a Compromise on Sleep?

Next, we will try to answer whether increased activity results in a
compromise on sleep quality or duration.

``` r
combined_plot("TotalActMin","MinAsleep","Total Active Minutes","Total Minutes Asleep")
```

<img src="Fitbit_DetailedAnalysis_For_Bellabeat_files/figure-gfm/minasleep_vs_totalactmin-1.png" alt="" style="display: block; margin: auto;" />
Higher activity levels do not appear to negatively impact sleep quality.
Instead, the scatter plots indicate that decreasing sedentary behavior
can free up time, leading to both increased activity and improved sleep.

#### What Influences Calorie Expenditure

Next, we will try to identify the primary factors that influence calorie
expenditure? Is there a difference in the impact of Activity on Calories
between weekdays and weekends

**Calories and Minutes Asleep**

Let’s first see if higher sleep quality leads to an increase or
reduction in Calorie Expenditure.

``` r
combined_plot("MinAsleep","Calories","Total Minutes Asleep","Calories (kcal)")
```

<img src="Fitbit_DetailedAnalysis_For_Bellabeat_files/figure-gfm/cal_vs_minasleep-1.png" alt="" style="display: block; margin: auto;" />
Interestingly, higher sleep duration has no impact on Calorie
Expenditure

**Calories and Total Steps**

Let’s now see if a higher step count leads to an increase in Calorie
Expenditure.

``` r
combined_plot("Steps","Calories","Total Steps","Calories (kcal)")
```

<img src="Fitbit_DetailedAnalysis_For_Bellabeat_files/figure-gfm/cal_vs_steps-1.png" alt="" style="display: block; margin: auto;" />
The data shows a strong correlation between a higher step count and a
significant increase in calorie expenditure. Bellabeat can leverage this
insight in its advertising or user notifications. For instance,
encouraging activities like running, walking, or using a treadmill can
serve as a direct incentive for users aiming to getting more fit and/or
achieving a calorie deficit.

**Calories and Activity**

Let’s now see if higher activity leads to an increase in Calorie
Expenditure and if there is a more pronounced effect on Weekends.

``` r
daytype_plot("TotalActMin","Calories","Total Active Minutes","Calories (kcal)")
```

<img src="Fitbit_DetailedAnalysis_For_Bellabeat_files/figure-gfm/cal_vs_totalactmin-1.png" alt="" style="display: block; margin: auto;" />
The relationship between activity and calorie expenditure differs
significantly between weekdays and weekends. While a positive
correlation exists on both, the line representing the relationship is
notably steeper on weekends than on weekdays, meaning that each
additional minute of activity results in a higher caloric burn on
weekends.

This difference may be due to the nature of the activity. Weekend
activity is often deliberate and sustained (e.g., long walks, gym
sessions, sports), which leads to a predictable, linear, and higher
calorie cost. Conversely, weekday calorie burn is more influenced by
less intense “background noise” such as stress, fidgeting, work
schedules, or incidental steps. These activities are less likely to
register fully as “Active Minutes,” resulting in a weaker relationship
between logged active time and calories burned.

Ultimately, this finding does not change Bellabeat’s core strategy; the
company can continue to use messaging similar to what they use for steps
to encourage users to improve their fitness and achieve a calorie
deficit. This messaging is likely to have a stronger effect on weekends,
as people are often participating in planned fitness activities. On
weekdays, the message functions as a helpful prompt for users to engage
in more physical activity

# Part 2: Hourly Level Analysis

Moving forward, we will now extend our analysis to a higher level of
granularity. We will also evaluate any impact of the type of the day
(Weekend vs Weekday) on fitness trends.

The subsequent analysis will focus on the hourly level data for
Intensities, Calories and Steps. Since a thorough analysis has already
been conducted at the daily level, we will bypass repeating this process
for the hourly analysis. Our primary goal for the hourly data will be to
review how the distributions of these indicators change throughout the
hours of the day.
...
AverageIntensity,Calories and Steps only exist in one of the tables
each. Let’s also review their descriptions to see if we should exclude
any among them. The table below lists descriptions for the relevant
columns from the Fitabase data dictionary available at this link:
...
- As discussed earlier, AverageIntensity is not included.
- TotalIntensity is renamed to Intensity
- StepTotal is renamed to Steps

``` sql
SELECT 
  I.Id, 
  I.ActivityHour,       
  I.TotalIntensity as Intensity,
...
The initial step involves generating summary statistics to facilitate a
more comprehensive understanding of the data set.
...
- The mean for Intensity is 12 minutes. This means that on average, in a
  given hour, respondents engage in 12 minutes of total activity.
...
- The mean of Calories burned is 97. This means that on average, in a
  given hour, respondents burn 97 calories
...
- We must first add two new columns: one representing the hour of the
  day (in 24-hour format) and another specifying the Day Type
  (differentiating between Weekend and Weekday)
- Next we will add some text for annotations at 6 pm and 1 pm.
- We will use a for loop to print plots for all 3 indicators (Intensity,
  Calories and Steps)
- This code computes the average Intensity/Calories/Steps for each hour
  of the day and then plots the results against time. Distinct plots are
  generated to compare weekday and weekend averages. A gray line is
  drawn at peak hours
...
for (i in c("Intensity","Calories","Steps")) {
  
  p <- hourly_activity %>% group_by(hour_of_day, daytype) %>% #For each hour
  summarise(hourly_avg = mean(.data[[i]]), .groups="drop") %>% #Calculate Avg 
  ...
  labs(title= paste0("Average ", i, " over Time"), # New Title
                              x='Hour of the Day (24-hr)', # New x and y labels
                              y=paste("Average ", i), fill="Day Type")
...
<img src="Fitbit_DetailedAnalysis_For_Bellabeat_files/figure-gfm/hourlyavg_time_day-1.png" alt="" style="display: block; margin: auto;" /><img src="Fitbit_DetailedAnalysis_For_Bellabeat_files/figure-gfm/hourlyavg_time_day-2.png" alt="" style="display: block; margin: auto;" /><img src="Fitbit_DetailedAnalysis_For_Bellabeat_files/figure-gfm/hourlyavg_time_day-3.png" alt="" style="display: block; margin: auto;" />

Intensity, Calorie Expenditure, and Steps exhibit identical patterns,
with distinct peaks occurring at the same time stamps on both weekdays
and weekend

- They are all observed to increase as the day progresses in charts for
  both day types before declining later.
- Distinct variations are also evident in the distributions for weekends
  and weekdays. On weekdays, all 3 indicators indicators peak between
  5-7 p.m., while on weekends, the peak is notably around 1 p.m.

This pattern is logical, as individuals on weekdays may engage in
planned physical activity (exercise/walks/sports) following the
conclusion of typical office hours at 5 p.m. Conversely, on weekends,
they have the flexibility to schedule these workouts earlier, such as at
1 p.m., a time that still permits a later start to the day.
...
The comprehensive analysis of the 33 Fitbit respondents yielded several
key insights and actionable recommendations for the Bellabeat Marketing
team. While these have been thoroughly detailed previously, the
following section provides a concise summary.

## Recommendations based on Daily Trends

Part 1 of this case study focused on **Daily** level data activity,
calories, intensities, steps and sleep. We find:

**Daily Calories**

- **Daily average for calories burned** is 2,361 kcal and it peaks at
  around 2000 kcal. While this aligns with standard dietary guidelines,
  it **is at the lower end of the 2000-2000 recommended range**. This
  suggests that many participants are not highly active, indicating a
  potential target audience for Bellabeat

**Daily Step Count, Distance and Activity**

- The average step count of 8,319 is within the typical 5,000–7,000 step
  range; however, the data reveals a user base with distinct segments of
  both high and low performers. High performers, who average more than
  10,000 steps per day, constitute more than one-fifth or (21.2%) of
  respondents, elevating the overall mean. Interestingly, the
  **proportion of Sedentary users, averaging less than 5,000 steps per
  day, is also the same i.e. 21.2% or slightly more than one fifth of
  the total repsondents**
- The analysis of **activity distance also reveals a significant
  variation** in respondent performance levels
- These disparate distribution patterns strongly indicate that a
  **one-size-fits-all approach is ineffective**, necessitating the
  development of a varied and targeted approach with user-targeted plans
  and personalized recommendations.
  - Bellabeat can target high performers with intensive workout plans
    and fitness goals to help them maintain their high steps counts
  - Bellabeat could consider targeting sedentary or low performers with
    daily reminders and low intense, easy workout plans to help them
    meet their daily goals.
- Overall, the data indicates that **most respondents are primarily
  lightly active** with Lightly Active Minutes accounting for
  approximately 85% of the total activity. This group represents a key
  target audience for Bellabeat, offering an opportunity to increase
  engagement by recommending workout plans and fitness goals.
- It will also be helpful for Bellabeat to account for any drop in
  activity during summer months by **promoting and recommending indoor
  workouts and activity options while the weather outside is hot**.

**Sedentary Behavior and Time Spent Sleeping**

- **Sedentary Minutes constitute a striking 79.4% of the total time**.
  This high proportion directly leads to a very high average of around
  956 minutes (or **16 hours**). The data also shows a Peak for
  Sedentary Hours at 12 hours which represents a very high level as
  compared to the recommended time limit of 300 minutes or 6 hours. The
  data reveals the presence of users who can be significantly inactive
  on certain days, with many spending over half their day in sedentary
  behavior. This presents a **key market opportunity** for Bellabeat to
  intervene by promoting reduced sedentary time. Bellabeat can address
  this by implementing targeted reminders to encourage activity when a
  user’s sedentary hours exceed a predetermined limit such as 6 hours.
  Effective suggestions like “Time to take a walk,” “Time for some
  stretches,” or “Time to Stand” could help transition sedentary users
  toward a more active lifestyle.

- The **average value for time spent sleeping is 419 minutes, which is
  approximately 7 hours** and aligns with the standard recommended
  value. The data also suggests that most people only sleep only sleep
  once in a day (presumably at night)

- The data also reveals users at the extremes of the distribution (with
  some sleeping less than 5 hours and some more than 9 hours). Bellabeat
  may **implement reminders or alarms to promote adherence to the
  recommended 7 hours and help users set up sleep schedules**.

## Recommendations based on Interrelationships

**Negative Impact of Sedentary Behavior on Sleep Quality and Activity**

- **Higher amount of sedentary behavior** is associated with **lower
  sleep quality and decreased activity** levels.
- The **Negative correlation between Minutes Asleep and sedentary
  minutes is also significantly stronger on weekends** than weekdays.
  This suggests that Bellabeat could benefit from promoting increased
  activity on weekends to help reduce sedentary behavior. Given these
  patterns, Bellabeat could implement the following:
  - Sleep Reminders: Send notifications to users to encourage sleep when
    late-night sedentary behavior is detected.
  - Screen Time Management: Track screen usage and send reminders to
    take breaks when use is prolonged.
- Based on the strong inverse relationship between Sedentary Behavior
  and activity, it is vital for Bellabeat to integrate actionable
  activity suggestions into reminders aimed at reducing sedentary time.
  For instance, messages could include: “You have been sitting for too
  long, time to take a walk,” or “Trouble having sleep? Time to put the
  phone down and do some stretches.”

**Reduce Sedentary Behavior without Comporomise on Sleep**

- Higher activity levels do not appear to negatively impact sleep
  quality. Decreasing sedentary behavior can free up time, leading to
  both increased activity and improved sleep. Bellabeat should emphasize
  this significant point in its advertisement and messaging.

**Recommendations to Increase Calorie Expenditure**

- Higher activity levels and step counts have appear to have a strong
  positive effect on calorie burning.
- Bellabeat can leverage this insight in its advertising or user
  notifications. For instance, encouraging activities like running,
  walking, or using a treadmill can serve as a direct incentive for
  users aiming to getting more fit and/or achieving a calorie deficit.
- The **Positive Correlation between Calories and Total Active Minutes
  is also much stronger on weekends**. This observation prompts the
  suggestion that Bellabeat should follow a more active policy on
  weekends to encourage users to boost calorie burn via higher activity.

## Recommendations based on Hourly Trends

Part 2 of the analysis focused on **Hourly** level data for calories,
intensities and steps. It examined how the distributions of these three
indicators fluctuate throughout the hours of the day.

- We find that on weekdays, all 3 indicators indicators peak between 5-7
  p.m, while on weekends, the peak occurs earlier, around 1 p.m.
- Therefore, reminders prompting users to engage in activity could be
  strategically timed differently for weekdays and weekends. On
  weekdays, reminders such as, “Time to take a walk,” or “Time to visit
  the gym,” could be dispatched at 5 p.m. On weekends, these prompts
  could be sent at around 12:45 p.m.

## Conclusion

Bellabeat has a significant opportunity to target users effectively and
increase its market share for its health and fitness products. The
analysis confirms the existence of a target demographic that can be
motivated to raise their calorie expenditure, boost their daily step
counts, and achieve their activity goals while reducing time spent being
sedentary, with the help of Bellabeat’s products and interventions.

**Personalization is Crucial**: The data shows considerable variation in
user performance and activity levels. A universal, “one-size-fits-all”
approach will be ineffective. Bellabeat should instead implement a
diversified, targeted strategy featuring personalized recommendations
and user-specific activity plans.

**Address High Sedentary Behavior**: Alarmingly high sedentary behavior
negatively impacts both activity and sleep quality. Bellabeat must
intervene by setting up targeted messages and reminders to prompt
activity when a user’s sedentary time exceeds a predetermined limit.

**Promote Step Count and Activity**: Messaging that encourages higher
activity levels and step counts should be integrated, as these
indicators were found to correlate with increased calorie burn.

**Time and Day Segmentation**: The effectiveness of messaging varies
significantly depending on the day type and timing: \* Weekends: A more
proactive policy is needed, as weekends show more distinct changes in
behavior. Messaging is likely to be most effective earlier in the day,
around 1 PM. \* Weekdays: Messaging is likely to be more effective later
